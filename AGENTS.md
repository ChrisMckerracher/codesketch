...
- Design UI hierarchy from user journeys and contextual relationships. An explicit from-zero UI reset discards prior presentation. For this reset, a separate fresh creative session must author the hierarchy without reading old layout proposals; existing layout prescriptions are superseded.
- Before substantial UI implementation, show the user a concrete mockup of the hierarchy and key user journey, request critique, and wait for explicit visual approval before delegating implementation; iterate mockups from feedback. Automated checks and other agents do not establish user design approval. Record technical verification and user visual acceptance separately when closing design or polish work.
...
- Design Codesketch as a polished Mac creative application. Visible Studio text uses the GLYPHS vector engine, fixed approved geometry, and invisible semantic controls for accessibility/input. The lead delegates visual implementation through `herdr`.
