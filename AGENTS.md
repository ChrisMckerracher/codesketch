...
- Painting agents begin with `node tools/paint.mjs guide`. Use the CLI for drawing, status, feedback, and PNG inspection through `view`; open its returned image path with the agent's image reader. Preserve a human pause until continuation is authorized.
