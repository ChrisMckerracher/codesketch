  ...
  <script>
    ...
    function renderStudio() {
      ...
      if (state.colorPickerOpen && !state.collapsed) {
        ...
        // 2D Saturation / Value Matrix of NICE SQUARES (matching FIRE drawing)
        let by = 202;
        const segW = 38;
        const segH = 15.5;
        for (let r = 0; r < 4; r++) {
          let bx = 486;
          for (let c = 0; c < 6; c++) {
            const cellColor = (state.pickerHue === 220) ? SV_BANDS[r][c] : hsvToHex(state.pickerHue, Math.round(((c + 0.5) / 6) * 100), Math.round((1 - (r + 0.5) / 4) * 100));
            draw_rect(bx, by, segW, Math.ceil(segH), cellColor);
            // Fine cell borders
            draw_rect(bx, by, segW, 1, "#0F172A", 0.15);
            draw_rect(bx, by, 1, Math.ceil(segH), "#0F172A", 0.15);
            bx += segW;
          }
          by += segH;
        }
        ...
        // Crosshair thumb in 2D field (snapped or aligned over active square)
        ...
        // 1D Hue Spectrum Slider Bar (Discrete color bands matching FIRE drawing)
        const hues = ["#EF4444", "#F97316", "#F59E0B", "#10B981", "#06B6D4", "#3B82F6", "#6366F1", "#A855F7", "#EC4899", "#EF4444"];
        let hx = 486;
        const hw = 228 / hues.length;
        for (const hc of hues) {
          draw_rect(hx, 270, Math.ceil(hw), 8, hc);
          hx += hw;
        }

        const hueThumbX = 486 + Math.round((state.pickerHue / 360) * 228);
        draw_ellipse(hueThumbX, 274, 14, 14, "#FFFFFF");
        draw_ellipse(hueThumbX, 274, 10, 10, hues[Math.min(hues.length - 1, Math.floor((state.pickerHue / 360) * hues.length))]);
      ...
      // 5. LIVE CANVAS EYEDROPPER LOUPE ("PIXEL THINGY"): CLEAR SQUARES, NOT GRADIENT!
      if (state.eyedropperActive && state.loupePos) {
        ...
        const gridSize = 7;
        const cellSize = 5;
        const gridW = gridSize * cellSize; // 35px
        const halfGrid = Math.floor(gridSize / 2); // 3
        const startX = Math.round(lx - gridW / 2);
        const startY = Math.round(ly - gridW / 2);

        // Outer dark frame with shadow
        draw_rect(startX - 3, startY - 3, gridW + 6, gridW + 6, "#0F172A");
        draw_rect(startX - 2, startY - 2, gridW + 4, gridW + 4, "#FFFFFF");
        draw_rect(startX - 1, startY - 1, gridW + 2, gridW + 2, "#0F172A");

        // Clear magnified pixel squares (NO GRADIENT!)
        for (let dy = -halfGrid; dy <= halfGrid; dy++) {
          for (let dx = -halfGrid; dx <= halfGrid; dx++) {
            ...
            try {
              ...
              cellCol = `#${toHex(pixel[0])}${toHex(pixel[1])}${toHex(pixel[2])}`.toUpperCase();
            } catch (e) {}

            const cx = startX + (dx + halfGrid) * cellSize;
            const cy = startY + (dy + halfGrid) * cellSize;
            draw_rect(cx, cy, cellSize, cellSize, cellCol);
            draw_rect(cx, cy, cellSize, 1, "#0F172A", 0.35);
            draw_rect(cx, cy, 1, cellSize, "#0F172A", 0.35);
          }
        }

        // Center target square reticle
        const centerCellX = startX + halfGrid * cellSize;
        const centerCellY = startY + halfGrid * cellSize;
        draw_rect(centerCellX, centerCellY, cellSize, 1, "#FFFFFF");
        draw_rect(centerCellX, centerCellY + cellSize - 1, cellSize, 1, "#FFFFFF");
        draw_rect(centerCellX, centerCellY, 1, cellSize, "#FFFFFF");
        draw_rect(centerCellX + cellSize - 1, centerCellY, 1, cellSize, "#FFFFFF");

        // Crosshairs extending out
        draw_stroke([[lx, startY - 8], [lx, startY - 2]], "#FFFFFF", 1.5);
        draw_stroke([[lx, startY + gridW + 2], [lx, startY + gridW + 8]], "#FFFFFF", 1.5);
        draw_stroke([[startX - 8, ly], [startX - 2, ly]], "#FFFFFF", 1.5);
        draw_stroke([[startX + gridW + 2, ly], [startX + gridW + 8, ly]], "#FFFFFF", 1.5);

        // Badge underneath with hex code
        draw_rounded_rect(lx - 24, startY + gridW + 8, 48, 14, 2, "#0F172A", 0.9);
        draw_text(col.toUpperCase(), lx - 21, startY + gridW + 10, 0.55, "#38BDF8", 1);

        // Leader line to pick button if popover is open
        if (state.colorPickerOpen) {
          draw_stroke([[lx + 18, ly], [486, 300]], "#0284C7", 1, 0.6);
        }
      }
    }
    ...
    field2D.onpointerdown = (e) => {
      ...
      const update = (clientX, clientY) => {
        ...
        const colIdx = Math.max(0, Math.min(5, Math.floor(((cx - 486) / 228) * 6)));
        const rowIdx = Math.max(0, Math.min(3, Math.floor(((cy - 202) / 62) * 4)));
        state.pickerSat = Math.round(((colIdx + 0.5) / 6) * 100);
        state.pickerVal = Math.round((1 - (rowIdx + 0.5) / 4) * 100);
        if (state.pickerHue === 220) {
          state.pickerColor = SV_BANDS[rowIdx][colIdx];
        } else {
