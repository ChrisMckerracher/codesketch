// artifacts/tumblr-faithful-sketch.mjs
// Faithful graphite pencil sketch replicating start_lol.png in the style of art_inspo.png.
// NO COLORING. Pure pencil sketch on off-white paper.
// Real Monferno anatomy (blue mask, flame tuft, ears, white ruff, arm rings, X X eyes, curled flame tail).
// Exact speech bubble: "I can SEE the sweat pouring out of your pokeballs bro".

import { writeFileSync } from 'node:fs';

// Subtle natural human hand jitter
function humanJitter(points, intensity = 0.55) {
  return points.map(([x, y], idx) => {
    const seed = (x * 17.13 + y * 53.47 + idx * 29.81);
    const wobbleX = (Math.sin(seed) * 0.7 + Math.cos(seed * 2.3) * 0.3) * intensity;
    const wobbleY = (Math.cos(seed * 1.9) * 0.7 + Math.sin(seed * 3.1) * 0.3) * intensity;
    return [
...
function sketchStroke(layer, ctrlPoints, { color = '#383028', size = 3.5, opacity = 0.9, wobble = 0.5 } = {}) {
  ...
  return [{ type: 'stroke', layer, brush: 'pencil', color, size, opacity, points: humanJitter(pts, wobble) }];
}

// Sketchy double stroke: a primary pass and a light, slightly offset second stroke
function doubleStroke(layer, ctrlPoints, opts = {}) {
  const c1 = sketchStroke(layer, ctrlPoints, opts);
  const offsetPoints = ctrlPoints.map(([x, y]) => [x + 0.5, y + 0.4]);
  const c2 = sketchStroke(layer, offsetPoints, {
    ...opts,
    size: Math.max(1.8, (opts.size || 3.5) * 0.72),
    opacity: (opts.opacity || 0.9) * 0.45,
    wobble: (opts.wobble || 0.5) * 1.3,
  });
  return [...c1, ...c2];
}

// Graphite pencil hatching for shadows
function sketchHatch(layer, x1, y1, x2, y2, angleDeg = 48, spacing = 4.2, length = 11, { color = '#453a32', size = 1.8, opacity = 0.45 } = {}) {
  const commands = [];
  ...
  for (let i = 0; i <= stepCount; i++) {
    ...
    const sx = x1 + (x2 - x1) * t;
    const sy = y1 + (y2 - y1) * t;
    const pts = humanJitter([[sx, sy], [sx + dx, sy + dy]], 0.35);
...
function buildPass1Arena() {
  const commands = [
    { type: 'fill', color: '#f4f1ea' }, // warm sketchbook paper
  ...
  // Horizontal step tiers across background
  commands.push(
    ...doubleStroke('arena', [[0, 235], [1000, 235]], { color: '#4a4038', size: 3.2 }),
    ...doubleStroke('arena', [[0, 265], [1000, 265]], { color: '#4a4038', size: 3.2 }),
    ...doubleStroke('arena', [[0, 295], [1000, 295]], { color: '#4a4038', size: 3.2 }),
    // Curved tier arcs
    ...doubleStroke('arena', [[0, 315], [250, 332], [500, 338], [750, 332], [1000, 315]], { color: '#4a4038', size: 3.4 }),
    ...doubleStroke('arena', [[0, 385], [250, 402], [500, 408], [750, 402], [1000, 385]], { color: '#4a4038', size: 3.4 }),
    ...doubleStroke('arena', [[0, 455], [250, 476], [500, 485], [750, 476], [1000, 455]], { color: '#4a4038', size: 3.4 }),
    ...doubleStroke('arena', [[0, 635], [250, 668], [500, 678], [750, 668], [1000, 635]], { color: '#4a4038', size: 3.4 })
  );

  // Central battle ring
  const cx = 500, cy = 528, rx = 84, ry = 22, orx = 185, ory = 42;
  ...
  const hexOuter = [[500, 88], [558, 122], [558, 188], [500, 222], [442, 188], [442, 122], [500, 88]];
  const hexInner = [[500, 118], [538, 138], [538, 172], [500, 192], [462, 172], [462, 138], [500, 118]];
  commands.push(
  ...
  commands.push(
    ...doubleStroke('arena', [[442, 188], [436, 235], [464, 235], [474, 222], [526, 222], [536, 235], [564, 235], [558, 188]], { color: '#382f28', size: 3.5 })
  );

  // Pillars / struts
  commands.push(
    ...doubleStroke('arena', [[275, 75], [275, 195]], { color: '#4a4038', size: 3.5 }),
    ...sketchStroke('arena', [[275, 120], [310, 75]], { color: '#4a4038', size: 3.2 }),
    ...doubleStroke('arena', [[635, 75], [665, 195]], { color: '#4a4038', size: 3.5 }),
    ...sketchStroke('arena', [[658, 120], [625, 75]], { color: '#4a4038', size: 3.2 })
  );
  ...
  commands.push(
    ...sketchHatch('arena', 20, 235, 255, 235, 50, 6, 9, { color: '#685c52', opacity: 0.4 }),
    ...sketchHatch('arena', 745, 235, 980, 235, 50, 6, 9, { color: '#685c52', opacity: 0.4 }),
    ...sketchHatch('arena', 446, 188, 554, 188, 60, 5, 8, { color: '#594d42', opacity: 0.45 })
  );
...
function buildPass2Trainers() {
  ...
  // Head, Hair, Glasses, Angry Profile Expression (Lucas with Messy Hair, NO HAT!)
  commands.push(
    // Neck
...
// ====================================================
// PASS 3: Knocked-Out Monferno (Accurate Pokémon Anatomy)
// ====================================================
