...
import (
	...
	"github.com/ChrisMckerracher/codesketch/internal/cli/input"
	"github.com/ChrisMckerracher/codesketch/internal/cli/parse"
	"github.com/ChrisMckerracher/codesketch/internal/cli/transport"
)

func feedbackText(parts []string) (string, error) {
	return label(strings.Join(parts, " "), "feedback", 2000)
}
...
func (r Runner) execute(ctx context.Context, c *transport.Client, command string, a *parse.Result, j invocation) error {
	...
	case "submit", "load":
		...
		if command == "load" {
			...
			if err := json.Unmarshal(data, &object); err != nil || object == nil {
				return usage("load requires a JSON project object")
			}
			j.path, j.body, j.label = "/api/project", json.RawMessage(data), "Loaded project"
		} else {
			commands, err := submitted(data)
			if err != nil {
				return err
			}
			j.path, j.body, j.label = "/api/commands", batch(commands, a), fmt.Sprintf("Queued %d command(s)", len(commands))
		}
	case "save":
	...
	case "feedback":
		if a.Booleans["json"] {
			return r.output(s.Feedback)
		}
		return r.text(formatFeedback(s.Feedback))
	case "layer":
		if a.Booleans["json"] {
