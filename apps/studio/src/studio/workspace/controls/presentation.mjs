...
function paintArea(ctx, v, entry, scrolls) {
  ...
  const layout = layoutText(v, source, innerWidth, 1);
  ...
  const maxScroll = Math.max(layout.height - innerHeight, 0);
  let scroll = Math.max(Number(scrolls.get(d.id)) || 0, 0);
  if (entry.focused) scroll = reveal(caret.y, layout.lineHeight, innerHeight, scroll);
  ...
  if (!empty && entry.focused && selectionEnd > selectionStart) {
    paintSelection(v, cells, selectionStart, selectionEnd, innerX, innerY, scroll, layout.lineHeight);
  }
  const opacity = d.disabled ? DISABLED_OPACITY : 1;
  const color = empty ? PLACEHOLDER_COLOR : TEXT_COLOR;
  for (let index = 0; index < cells.length; index += 1) {
    const cell = cells[index];
    if (cell.char === "\n" || cell.y + layout.lineHeight < scroll) continue;
    if (cell.y - scroll > innerHeight) break;
    if (cell.char) v.text(cell.char, innerX + cell.x, innerY + cell.y - scroll, 1, color, 1, opacity);
  }
...
function paintSelection(v, cells, start, end, innerX, innerY, scroll, lineHeight) {
  for (let index = 0; index < cells.length; index += 1) {
    const cell = cells[index];
