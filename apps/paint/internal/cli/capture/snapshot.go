...
func validateSnapshot(raw json.RawMessage, options Options) (captureData, error) {
	...
	if len(envelope.Document) == 0 || envelope.Revision == nil || envelope.Playback == nil || len(envelope.Playback.Active) == 0 {
		return data, errors.New("snapshot requires document, instanceId, revision and playback.active")
	}
	if err := json.Unmarshal(envelope.Document, &data.Document); err != nil {
		return data, fmt.Errorf("invalid document: %w", err)
	}
	data.InstanceID, data.Revision = envelope.InstanceID, *envelope.Revision
	if data.InstanceID == "" || len(data.InstanceID) > 200 || data.Revision < 0 || data.Revision > 9_007_199_254_740_991 {
		return data, errors.New("invalid snapshot identity or revision")
	}
	if !options.Committed {
		var err error
		data.Active, err = previewActive(envelope.Playback.Active)
		if err != nil {
			return data, err
		}
	}
