...
var descriptions = map[string]string{
	...
	"comments":   "comments [list|wait|watch|ack|address|reply]\nList human comments or follow them live. list (default) prints anchored comments and replies with the cursor, generation, and epoch needed for follow-up. wait blocks for the next comments change, reset, or control change; timeout 0.001..30 seconds (default 30) exits 1 with COMMENTS_TIMEOUT and never resumes playback. watch streams the initial envelope then change events (compact NDJSON with --json); its deadline exits 0. ack and address ID confirm a comment with --generation STRING and --seq N; reply ID TEXT --generation STRING --seq N --request-id ID posts an agent reply. Reply text is 1..2000 nonblank characters and request IDs are 1..80 characters; stale 409 conflicts surface as errors without auto-refresh.",
