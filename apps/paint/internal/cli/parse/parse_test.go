...
// no-open is a hyphenated boolean; both flag forms reject values and
// duplicates through the shared boolean handling.
func TestNoOpenBoolean(t *testing.T) {
	got, err := Args([]string{"--no-open", "start"}, []string{"no-open"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !got.Booleans["no-open"] || !reflect.DeepEqual(got.Positionals, []string{"start"}) {
		t.Fatalf("Flags=%v Booleans=%v Positionals=%v", got.Flags, got.Booleans, got.Positionals)
	}
	for _, tt := range []struct {
		...
		{[]string{"-no-open"}, nil},
		{[]string{"--no-open", "start"}, []string{"start"}},
		{[]string{"start", "-no-open"}, []string{"start"}},
	} {
		got, err = Args(tt.argv, []string{"no-open"})
		if err != nil {
			t.Fatalf("Args(%v) unexpected error: %v", tt.argv, err)
		}
		if !got.Booleans["no-open"] || !reflect.DeepEqual(got.Positionals, tt.want) {
			t.Fatalf("Args(%v): Booleans=%v Positionals=%v, want positionals %v", tt.argv, got.Booleans, got.Positionals, tt.want)
		}
	}
	for _, argv := range [][]string{{"--no-open=true"}, {"-no-open=false"}, {"--no-open", "--no-open"}, {"--no-open", "-no-open"}} {
		if _, err := Args(argv, []string{"no-open"}); err == nil {
			t.Fatalf("Args(%v) accepted an invalid no-open invocation", argv)
		}
	}
}
